...
for scene_idx in range(100, 130):
    for anno_idx in range(0, 255, int(1/sample_ratio)):
        ...
        for batch_idx in range(B):
            ...
            # umap_geo = umap.UMAP(n_neighbors=50, min_dist=0.1, metric='euclidean', random_state=random_seed)
            # umap_geo_emb = umap_geo.fit_transform(geo_inst_feat)  # [N, 2]

            # plt.figure(figsize=(8, 6))
            # sc = plt.scatter(umap_geo_emb[:, 0], umap_geo_emb[:, 1], s=12, c=geo_inst_graspness, cmap='viridis', alpha=0.8)
            # plt.colorbar(sc, label='Graspness')
            # plt.title('t-SNE Features Colored by Graspness Value')
            # plt.axis('off')
            # plt.tight_layout()
            # plt.savefig(os.path.join(vis_root, 'feature_vis', 'scene_{:04d}_anno_{:04d}_batch_{}_point_graspness_umap.png'.format(scene_idx, anno_idx, batch_idx)), dpi=300)
            # plt.close()

            # umap_rgb = umap.UMAP(n_neighbors=50, min_dist=0.1, metric='euclidean', random_state=random_seed)
            # umap_rgb_emb = umap_rgb.fit_transform(rgb_inst_feat)  # [N, 2]

            # plt.figure(figsize=(8, 6))
            # sc = plt.scatter(umap_rgb_emb[:, 0], umap_rgb_emb[:, 1], s=12, c=rgb_inst_graspness, cmap='viridis', alpha=0.8)
            # plt.colorbar(sc, label='Graspness')
            # plt.title('t-SNE Features Colored by Graspness Value')
            # plt.axis('off')
            # plt.tight_layout()
            # plt.savefig(os.path.join(vis_root, 'feature_vis', 'scene_{:04d}_anno_{:04d}_batch_{}_rgb_graspness_umap.png'.format(scene_idx, anno_idx, batch_idx)), dpi=300)
            # plt.close()

            # # # 合并特征，统一进行t-SNE降维
            # combined_features = np.concatenate([rgb_inst_feat, geo_inst_feat], axis=0)

            # umap_comb = umap.UMAP(n_neighbors=50, min_dist=0.1, metric='euclidean', random_state=random_seed)
            # combined_embedded = umap_comb.fit_transform(combined_features)
            ...
            # plt.savefig(os.path.join(vis_root, 'feature_vis', 'scene_{:04d}_anno_{:04d}_batch_{}_umap.svg'.format(scene_idx, anno_idx, batch_idx)), format='svg', dpi=600)
            # plt.savefig(os.path.join(vis_root, 'feature_vis', 'scene_{:04d}_anno_{:04d}_batch_{}_umap.png'.format(scene_idx, anno_idx, batch_idx)), dpi=600)
            # plt.close()
            ...
            umap_geo = umap.UMAP(n_neighbors=50, min_dist=0.1, metric='euclidean', random_state=random_seed)
            umap_geo_emb = umap_geo.fit_transform(geo_inst_feat_norm)  # [N, 2]
            umap_rgb = umap.UMAP(n_neighbors=50, min_dist=0.1, metric='euclidean', random_state=random_seed)
            umap_rgb_emb = umap_rgb.fit_transform(rgb_inst_feat_norm)  # [N, 2]

            # --- 并排可视化 ---
            fig, axes = plt.subplots(1, 2, figsize=(12, 6), sharex=True, sharey=True)

            sc0 = axes[0].scatter(umap_geo_emb[:, 0], umap_geo_emb[:, 1], s=12, c=geo_inst_graspness, cmap='viridis', alpha=0.8)
            axes[0].set_title('Point-only Baseline Feature', fontsize=14, fontweight='bold')
            axes[0].axis('off')
            sc1 = axes[1].scatter(umap_rgb_emb[:, 0], umap_rgb_emb[:, 1], s=12, c=rgb_inst_graspness, cmap='viridis', alpha=0.8)
            ...
            plt.savefig(os.path.join(
                ...
                f'scene_{scene_idx:04d}_anno_{anno_idx:04d}_batch_{batch_idx}_umap_sidebyside.png'
            ), dpi=600)
            plt.savefig(os.path.join(
                vis_root, 'feature_vis', 
                f'scene_{scene_idx:04d}_anno_{anno_idx:04d}_batch_{batch_idx}_umap_sidebyside.svg'
            ), format='svg', dpi=600)
            plt.close()