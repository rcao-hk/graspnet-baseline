import os
# Do not force CUDA_LAUNCH_BLOCKING here. The profiler synchronizes explicitly.
...
from inference_cost_profiler import (
    InferenceCostProfiler,
    add_inference_profile_args,
    collect_model_metadata,
    cuda_synchronize,
    timed_call,
)
from fusion_profile_utils import clone_tensor_tree
...
# Canonical arguments shared with train_profile.sh / inference_profile.sh.
add_inference_profile_args(parser)

# Backward-compatible aliases used by the original timing driver.
parser.add_argument(
    '--profile_inference',
    dest='profile_cost',
    action='store_true',
...
parser.add_argument(
    '--profile_breakdown',
    dest='profile_stage_breakdown',
    action='store_true',
    help=argparse.SUPPRESS,
)
parser.add_argument(
    '--enable_inference_timer',
    action='store_true',
    help=(
        'Deprecated alias that enables --profile_cost and '
...
if cfgs.enable_inference_timer:
    cfgs.profile_cost = True
    cfgs.profile_stage_breakdown = True
if cfgs.profile_stage_breakdown or cfgs.profile_complexity:
    cfgs.profile_cost = True
if cfgs.profile_warmup < 0 or cfgs.profile_samples < 0:
    parser.error('--profile_warmup and --profile_samples must be non-negative.')
if cfgs.sample_interval <= 0:
    parser.error('--sample_interval must be positive.')
...
if cfgs.profile_stage_breakdown:
    if not hasattr(net, 'enable_inference_timer'):
        raise AttributeError(
            f"{type(net).__name__} does not implement enable_inference_timer(). "
            "Please use the MMGNet model file with inference timer support."
        )
    net.enable_inference_timer(
        enabled=True,
        warmup=cfgs.profile_warmup,
        sync_cuda=True,
        attach_to_end_points=False,
        reset=True,
    )
    if hasattr(net, 'enable_grouping_timer'):
        net.enable_grouping_timer(
            enabled=True,
            warmup=cfgs.profile_warmup,
            sync_cuda=True,
            reset=True,
    ...
    print(
        '[DEPLOY-PROFILE][WARN] Internal stage timing is enabled. '
        'Per-stage CUDA synchronization inflates total latency. Use a separate '
        'run without --profile_stage_breakdown for the primary latency table.'
    )
else:
    if hasattr(net, 'enable_inference_timer'):
        net.enable_inference_timer(enabled=False, reset=True)
    if hasattr(net, 'enable_grouping_timer'):
        net.enable_grouping_timer(enabled=False, reset=True)

eps = 1e-8

profile_output_dir = cfgs.profile_output_dir or os.path.join(
    dump_dir, 'deployment_profile'
)
model_metadata = collect_model_metadata(net, ckpt_name, device)
if cfgs.profile_cost:
    checkpoint_mb = model_metadata.get('checkpoint_size_mb') or 0.0
    print('[DEPLOY-PROFILE] Model metadata:')
    print(
        f"  parameters={model_metadata['total_params']:,} "
        f"({model_metadata['model_tensor_size_mb']:.2f} MiB tensors), "
        f"checkpoint={checkpoint_mb:.2f} MiB"
    )

controlled_protocol = {
    'dataset': {
        'name': 'GraspNet-1Billion',
        'root': os.path.abspath(cfgs.dataset_root),
        'split': cfgs.split,
        'camera': cfgs.camera,
        'data_type': cfgs.data_type,
        'restored_depth': bool(cfgs.restored_depth),
        'sample_interval': int(cfgs.sample_interval),
        'random_seed': 0,
    },
    'corruptions': {
        ...
        'pc_sparse_level': int(cfgs.pc_sparse_level),
    },
    'input': {
        'batch_size': 1,
        ...
        'image_size': list(resize_shape),
        'workspace_crop': True,
    },
    'postprocessing': {
        'prediction_decode': True,
        'graspgroup_construction': True,
        'collision_detection': bool(cfgs.collision_thresh > 0),
        'collision_thresh': float(cfgs.collision_thresh),
        'collision_voxel_size': float(cfgs.collision_voxel_size),
        'approach_dist': 0.05,
    },
    'measurement': {
        'warmup_samples': int(cfgs.profile_warmup),
        'profiled_samples': int(cfgs.profile_samples),
        'sync_cuda': True,
        'precision': 'fp32',
        'amp_enabled': False,
        'save_time_reported_separately': True,
        'stage_breakdown_intrusive': bool(cfgs.profile_stage_breakdown),
    },
}
...
run_metadata = {
    ...
    'voxel_size': cfgs.voxel_size,
    'image_height': int(resize_shape[0]),
    'image_width': int(resize_shape[1]),
    'image_size': list(resize_shape),
    'batch_size': 1,
    'precision': 'fp32',
    'amp_enabled': False,
    ...
    'voxel_size_cd': cfgs.collision_voxel_size,
    'profile_complexity': bool(cfgs.profile_complexity),
    'profile_stage_breakdown': bool(cfgs.profile_stage_breakdown),
    'cuda_launch_blocking': os.environ.get('CUDA_LAUNCH_BLOCKING'),
    'controlled_protocol': controlled_protocol,
}

inference_profiler = InferenceCostProfiler(
    enabled=cfgs.profile_cost,
    warmup=cfgs.profile_warmup,
    max_profiled_samples=cfgs.profile_samples,
    output_dir=profile_output_dir,
    device=device,
    print_every=cfgs.profile_print_every,
    sync_cuda=True,
    model=net,
...
save_run_config_json(
    ...
    extra={
        ...
        "seed": 0,
        "model_metadata": model_metadata,
        "profile_output_dir": os.path.abspath(profile_output_dir),
    }
)
...
def _profile_complexity_once(raw_model_inputs, scene_idx, anno_idx):
    """Run one diagnostic forward after latency/memory collection is complete."""
    if not (cfgs.profile_cost and cfgs.profile_complexity):
        return
    if inference_profiler.complexity is not None:
        return
    if inference_profiler.complexity_profile_error is not None:
        return

    # IGNet mutates the endpoint dictionary and may replace input entries such
    # as ``point_clouds``. The caller reconstructs this dictionary from the
    # original local tensors after timing, then we clone it for the hook pass.
    complexity_input = clone_tensor_tree(raw_model_inputs)
    try:
        print(
            '[DEPLOY-PROFILE] Profiling forward MACs/FLOPs on '
            f'scene={scene_idx:04d}, anno={anno_idx:04d}.'
        )
        with torch.inference_mode():
            inference_profiler.profile_forward_complexity(
                net,
                lambda: net(complexity_input),
                label=(
                    f'inference_forward_scene_{scene_idx:04d}_'
                    f'anno_{anno_idx:04d}'
                ),
            )
    except Exception as exc:
        # Latency/memory results remain valid if sparse-MAC hooks are not
        # supported by the local MinkowskiEngine build. The JSON records the
        # exact exception in complexity_profile_error.
        print(
            '[DEPLOY-PROFILE][WARN] Complexity profiling failed: '
...
def inference(scene_idx, *, is_last_scene=False):
    interval = int(max(1, cfgs.sample_interval))
    last_selected_anno = (255 // interval) * interval
    ...
    for anno_idx in range(256):
        ...
        # Synchronize after host-to-device construction so preprocessing time
        # includes pending transfers rather than charging them to the forward.
        if cfgs.profile_cost:
            cuda_synchronize(device)
        preprocess_ms = (time.perf_counter() - frame_start_time) * 1000.0

        memory_state = inference_profiler.start_online_memory_tracking()
        if cfgs.profile_cost:
            cuda_synchronize(device)
        ...
        with torch.inference_mode():
            if cfgs.profile_cost:
                end_points, model_forward_ms = timed_call(
                    lambda: net(batch_data_label), device=device, sync_cuda=True
                )
            else:
            ...
            if cfgs.profile_cost:
                grasp_preds, pred_decode_ms = timed_call(
                    lambda: pred_decode(end_points), device=device, sync_cuda=True
                )
            else:
            ...
            preds = grasp_preds[0]
            if cfgs.profile_cost:
                gg, graspgroup_ms = timed_call(
                    lambda: GraspGroup(preds), device=device, sync_cuda=False
                )
            else:
                gg = GraspGroup(preds)
                graspgroup_ms = 0.0

        num_grasps_raw = len(gg)
        collision_ms = 0.0
        if cfgs.collision_thresh > 0:
            def _run_collision_filter():
                detector = ModelFreeCollisionDetectorTorch(
                    cloud.reshape(-1, 3), voxel_size=cfgs.collision_voxel_size
                )
                cur_mask = detector.detect(
                    gg,
                    approach_dist=0.05,
                    collision_thresh=cfgs.collision_thresh,
                )
                cur_mask = cur_mask.detach().cpu().numpy()
                return gg[~cur_mask]

            if cfgs.profile_cost:
                gg, collision_ms = timed_call(
                    _run_collision_filter, device=device, sync_cuda=True
                )
            else:
                gg = _run_collision_filter()

        if cfgs.profile_cost:
            cuda_synchronize(device)
        online_inference_ms = (time.perf_counter() - online_start_time) * 1000.0
        memory_metrics = inference_profiler.finish_online_memory_tracking(memory_state)
        num_grasps_after_collision = len(gg)
        ...
        # Save is measured separately and excluded from online inference.
        ...
        save_ms = (time.perf_counter() - save_start_time) * 1000.0
        end_to_end_with_save_ms = (time.perf_counter() - frame_start_time) * 1000.0
        sensor_to_grasp_ms = preprocess_ms + online_inference_ms
        ...
        row = {
            ...
            'num_valid_points_before_sampling': int(len(cloud_masked)),
            'num_grasps_raw': int(num_grasps_raw),
            'num_grasps_after_collision': int(num_grasps_after_collision),
            'preprocess_ms': float(preprocess_ms),
            'model_forward_ms': float(model_forward_ms),
            'pred_decode_ms': float(pred_decode_ms),
            'graspgroup_ms': float(graspgroup_ms),
            'collision_ms': float(collision_ms),
            'online_inference_ms': float(online_inference_ms),
            'sensor_to_grasp_ms': float(sensor_to_grasp_ms),
            'save_ms': float(save_ms),
            'end_to_end_with_save_ms': float(end_to_end_with_save_ms),
            **memory_metrics,
        }
        stop_requested = inference_profiler.add_row(row, model=net)
        is_natural_final_sample = bool(
            is_last_scene and anno_idx == last_selected_anno
        )

        print('Saving {}, {}'.format(scene_idx, anno_idx))
        if stop_requested or is_natural_final_sample:
            # Reconstruct pristine inputs from the original local tensors. IGNet
            # mutates ``batch_data_label`` during its measured forward.
            raw_model_inputs = {
            ...
            # Run diagnostic complexity only after the final measured memory row
            # has been committed, so hook allocations cannot contaminate latency
            # or peak-memory statistics.
            _profile_complexity_once(raw_model_inputs, scene_idx, anno_idx)
        if stop_requested:
            print('[DEPLOY-PROFILE] Requested number of measured frames reached.')
            return True
...
# scene_list = [100]
# res = []
for scene_pos, scene_idx in enumerate(scene_list):
    should_stop = inference(
        scene_idx, is_last_scene=(scene_pos == len(scene_list) - 1)
    )
    if should_stop:
        break
...
if cfgs.profile_cost:
    extra_summary = {}
    if cfgs.profile_stage_breakdown:
        breakdown_stats = net.get_inference_timer(reset=False)
        breakdown_payload = {'model': breakdown_stats}
        extra_summary['instrumented_model_breakdown'] = breakdown_stats
        if hasattr(net, 'get_grouping_timer'):
            grouping_stats = net.get_grouping_timer(reset=False)
            breakdown_payload['grouping_operator'] = grouping_stats
            extra_summary['instrumented_grouping_breakdown'] = grouping_stats
        breakdown_path = os.path.join(
            profile_output_dir, 'inference_model_breakdown.json'
        )
        with open(breakdown_path, 'w', encoding='utf-8') as f:
            json.dump(
                breakdown_payload, f, indent=2, ensure_ascii=False, sort_keys=True
            )
        print('\n' + '=' * 88)
        print('[INFER-BREAKDOWN] Instrumented model-forward breakdown')
        print(net.format_inference_timer())
        print(f'[INFER-BREAKDOWN] Saved: {breakdown_path}')
        print('=' * 88)

    csv_path, json_path, profile_summary = inference_profiler.export(
        basename='inference_deployment_profile',
        extra_summary=extra_summary,
    )
    print('\n' + '=' * 104)
    print(InferenceCostProfiler.format_summary(profile_summary))
    print(f'[DEPLOY-PROFILE] Per-frame rows: {csv_path}')
    print(f'[DEPLOY-PROFILE] Summary:        {json_path}')
    print('=' * 104)
