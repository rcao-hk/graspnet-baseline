...
from data_utils import CameraInfo, create_point_cloud_from_depth_image, sample_points, apply_smoothing
...
def distance_compute(scene_idx, cfgs):
    ...
    with torch.no_grad():
        for anno_idx in range(256):
            ...
            clear_depth_mask = (clear_depth > 0) & foreground_mask
            real_depth_mask = (real_depth > 0) & foreground_mask
            if cfgs.smooth_level > 0:
                smooth_depth = apply_smoothing(clear_depth, size=cfgs.smooth_level)
            else:
                smooth_depth = clear_depth
            noisy_cloud = create_point_cloud_from_depth_image(smooth_depth, camera_info, organized=True)
            real_cloud = create_point_cloud_from_depth_image(real_depth, camera_info, organized=True)
            
            noisy_cloud = noisy_cloud[clear_depth_mask]
            real_cloud = real_cloud[real_depth_mask]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--smooth_level', type=int, default=0, help=' [default: 0]')
    ...
    np.save(os.path.join(save_root, 'smooth_{}_depth_distance.npy'.format(cfgs.smooth_level)), results)