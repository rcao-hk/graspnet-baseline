...
object_pose[:3, 3] = np.array([0, 0, 0.5])
...
rendered_color = image_render(meshes_transed, cam_param, output_name=output_name, width=img_length, height=img_width, is_offscreen=opt.is_offscreen)

# bbox, pad_bbox, center = get_bbox(inst_mask, object_pose[:3, 3], intrinsics)
# (rmin, rmax, cmin, cmax) = bbox
# (pad_top, pad_bottom, pad_left, pad_right) = pad_bbox

# patch_color = rendered_color[rmin:rmax, cmin:cmax, :]
...
# patch_color = rendered_color[rmin:rmax, cmin:cmax, :]
patch_color = rendered_color

patch_color = cv2.cvtColor(patch_color, cv2.COLOR_BGR2RGB)
cv2.imwrite(os.path.join(opt.output_path, "{}_{}_rendered.png".format(anno_id, choose_idx)), patch_color)
...
for angle in [30, 60, 90]:
    ...
    rendered_rotated_img = rotate_image(patch_color, -rotation_angle, fill_color=(0, 0, 0))
