...
def distance_compute(scene_idx, cfgs):
    ...
    for anno_idx in range(256):
        ...
        # depth = apply_smoothing(depth, size=29)
        if cfgs.smooth_size > 1:
            noise_depth = apply_smoothing(clear_depth, size=cfgs.smooth_size)
        else:
            noise_depth = clear_depth
            
        if cfgs.gaussian_noise_level > 0:
            noisy_depth = add_gaussian_noise_depth_map(noise_depth, factor_depth, level=cfgs.gaussian_noise_level, valid_min_depth=0.1)
        ...
        noisy_cloud_masked = noisy_cloud[mask]
        clear_cloud_masked = cloud[mask]
        ...
        if cfgs.dropout_num > 0:
            ...
            inst_cloud.append(noisy_cloud_masked[background_mask])
            inst_color.append(color_masked[background_mask])
            for obj_idx in obj_idxs:
                if (seg_masked == obj_idx).sum() < 50:
                    continue
                inst_mask = (seg_masked == obj_idx)
                inst_cloud_select, select_idx, dropout_idx = random_point_dropout(noisy_cloud_masked[inst_mask], min_num=50, num_points_to_drop=3, radius_percent=0.1)
                inst_cloud.append(inst_cloud_select)
                inst_color.append(color_masked[inst_mask][select_idx])
            noisy_cloud_masked = np.concatenate(inst_cloud, axis=0)
            color_masked = np.concatenate(inst_color, axis=0)

        clear_cloud = o3d.geometry.PointCloud()
        clear_cloud.points = o3d.utility.Vector3dVector(clear_cloud_masked)
        clear_cloud = clear_cloud.voxel_down_sample(voxel_size=cfgs.voxel_size)
        ...
        clear_cloud_tensor = torch.tensor(np.asarray(clear_cloud.points), dtype=torch.float32, device=device)
        ...
        # clear_dis, _ = chamfer_distance(clear_cloud_tensor.unsqueeze(0), real_cloud_tensor.unsqueeze(0))
