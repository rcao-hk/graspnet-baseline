...
class GraspNet(nn.Module):
    ...
    def forward(self, end_points):
        ...
        if point_num < self.M_points:
            self.M_points = point_num
...
class GraspNet_multimodal(nn.Module):
    def __init__(self,  num_view=300, num_angle=12, num_depth=4, cylinder_radius=0.05, seed_feat_dim=512, img_feat_dim=64, is_training=True, fuse_type='early'):
        super().__init__()
        self.is_training = is_training
        self.seed_feature_dim = seed_feat_dim
        self.num_depth = num_depth
        ...
        if self.fuse_type == 'early':
            ...
            print('GSNet, late fusion (concatentation)')
            ...
            print('GSNet, intermediate fusion (DeepViewAgg style)')
    ...
    def forward(self, end_points):
        # use all sampled point cloud, B*Ns*3
        seed_xyz = end_points['point_clouds']
        B, N, _ = seed_xyz.shape  # batch _size
        ...
        elif self.fuse_type in ['early', 'concat']:
            img = end_points['img']
        ...
        end_points = self.graspable(feat_full, end_points)
        seed_features_flipped = feat_full.transpose(1, 2)  # B*Ns*feat_dim
        objectness_score = end_points['objectness_score']
        graspness_score = end_points['graspness_score'].squeeze(1)
        objectness_pred = torch.argmax(objectness_score, 1)
        objectness_mask = (objectness_pred == 1)
        ...
        for i in range(B):
            ...
            if cur_idx.numel() == 0:
                ridx = torch.randint(0, N, (self.M_points,), device=seed_xyz.device)
