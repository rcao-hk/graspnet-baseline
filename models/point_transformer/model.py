...
from ..GSNet import RotationScoringNet, WidthNet, process_grasp_labels, match_grasp_view_and_label
class Ignet_pt(nn.Module):
    def __init__(
        self,
        num_view=300,
        num_angle=12,
        ...
        self.rotation_head = RotationScoringNet(self.num_view, num_angle=self.num_angle,
                                                ...
                                                seed_feature_dim=self.backbone_out_channels, 
    ...
    def forward(self, end_points):
        B, num_point, _ = end_points['point_clouds'].size()
        input_dict = {
        "coord": end_points['point_clouds'].reshape(-1, 3),
        "grid_size": self.voxel_size,
        "feat": end_points['feats'],
        "batch": end_points['coors'][:, 0].int(),
        }

        point = Point(input_dict)
        point = self.backbone(point)
        
        input_features = point.feat.reshape(B, num_point, self.backbone_out_channels).transpose(1, 2)
        end_points['seed_features'] = input_features
        end_points = self.rotation_head(input_features, end_points)
        end_points = self.width_head(input_features, end_points)
        if self.is_training:
