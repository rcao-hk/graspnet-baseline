...
from loss_utils import GRASP_MAX_WIDTH, GRASP_MAX_TOLERANCE, GRASPNESS_THRESHOLD
...
from backbone import Pointnet2Backbone, TransformerEncoderBackbone, MinkBackbone
...
class GraspNetStage1(nn.Module):
    def __init__(self, input_feature_dim=0, num_view=300):
        ...
        # self.backbone = TransformerEncoderBackbone(input_feature_dim)
        # self.backbone = MinkBackbone(input_feature_dim)
        self.vpmodule = ApproachNet(num_view, 256)

    def forward(self, end_points):
        pointcloud = end_points['point_clouds']  # bs x num x channels
        seed_features, seed_xyz, end_points = self.backbone(
            pointcloud, end_points)
...
class GraspNetStage2(nn.Module):
    ...
    def forward(self, end_points):
        ...
        if self.is_training:
            grasp_top_views_rot, _, _, _, end_points = match_grasp_view_and_label(
                end_points)
...
def pred_decode(end_points):
    ...
    for i in range(batch_size):
        ...
        grasp_preds.append(torch.cat([grasp_score, grasp_width, grasp_height,
                           grasp_depth, rotation_matrix, grasp_center, obj_ids], axis=-1))
    return grasp_preds
