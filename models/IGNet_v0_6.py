...
class RotationScoringNet(nn.Module):
    def __init__(self, num_view, num_angle, num_depth, seed_feature_dim, is_training=True):
        ...
        # # regression-based
        self.conv3 = nn.Conv1d(self.in_dim * 2, self.num_view * self.num_angle, 1)
...
class IGNet(nn.Module):
    def __init__(self,  num_view=300, num_angle=12, num_depth=4, seed_feat_dim=512, is_training=True):
        ...
        self.graspness_head = RotationScoringNet(self.num_view, num_angle=self.num_angle,
                                                num_depth=self.num_depth,
                                                seed_feature_dim=self.seed_feature_dim, 
                                                is_training=self.is_training)
        self.crop = CloudCrop(nsample=64, seed_feature_dim=self.seed_feature_dim)
    ...
    def forward(self, end_points):
        ...
        end_points = self.graspness_head(seed_features, end_points)
        # if self.is_training:
...
def process_grasp_labels(end_points):
    ...
    batch_grasp_rot_graspness = torch.sum(batch_grasp_scores, dim=-1)  # (B, Ns, V, A)
