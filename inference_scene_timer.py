...
parser.add_argument('--enable_inference_timer', action='store_true',
                    help='Enable model inference breakdown timing and print per-frame/final timing [default: False]')
parser.add_argument('--profile_inference', action='store_true',
                    help='Collect deployment-oriented inference latency, throughput, and memory metrics')
parser.add_argument('--profile_warmup', type=int, default=20,
                    help='Number of processed frames excluded as profiler warmup [default: 20]')
parser.add_argument('--profile_samples', type=int, default=200,
                    help='Number of non-warmup frames to profile; 0 means all selected frames [default: 200]')
parser.add_argument('--profile_output_dir', type=str, default=None,
                    help='Profiler output directory; default: <dump_dir>/deployment_profile')
parser.add_argument('--profile_print_every', type=int, default=20,
                    help='Print one profiler status line every N measured frames [default: 20]')
parser.add_argument('--profile_breakdown', action='store_true',
                    help='Also enable instrumented model-stage breakdown; this adds per-stage CUDA synchronization')
cfgs = parser.parse_args()

if cfgs.enable_inference_timer:
    cfgs.profile_inference = True
    cfgs.profile_breakdown = True
...
if cfgs.profile_breakdown:
    ...
    print(
        '[DEPLOY-PROFILE][WARN] Internal breakdown is enabled. '
        'Its per-stage synchronization increases total latency; use a separate '
        'run without --profile_breakdown for primary steady-state latency.'
    )
elif hasattr(net, 'enable_grouping_timer'):
    net.enable_grouping_timer(enabled=False, reset=True)

eps = 1e-8

profile_output_dir = cfgs.profile_output_dir or os.path.join(dump_dir, 'deployment_profile')
model_metadata = collect_model_metadata(net, ckpt_name, device)
if cfgs.profile_inference:
...
inference_profiler = InferenceCostProfiler(
    enabled=cfgs.profile_inference,
    warmup=cfgs.profile_warmup,
...
def inference(scene_idx):
    ...
    for anno_idx in range(256):
        ...
        with torch.inference_mode():
            ...
            if cfgs.profile_breakdown:
                last_timing = net.get_last_inference_timer()
                total_ms = last_timing['stages_ms'].get('model_total', 0.0)
                phase = 'warmup' if last_timing['is_warmup'] else 'profiled'
                print(
                    f"[INFER-BREAKDOWN][{phase}] scene={scene_idx:04d} "
                    f"anno={anno_idx:04d} instrumented_model_total={total_ms:.3f} ms"
                )
        ...
        stop_requested = inference_profiler.add_row(row)
...
for scene_idx in scene_list:
    should_stop = inference(scene_idx)
    if should_stop:
        break
...
if cfgs.profile_inference:
    ...
    if cfgs.profile_breakdown:
        ...
        breakdown_path = os.path.join(profile_output_dir, 'inference_model_breakdown.json')
        with open(breakdown_path, 'w', encoding='utf-8') as f:
            json.dump(breakdown_stats, f, indent=2, ensure_ascii=False, sort_keys=True)
        print('\n' + '=' * 88)
